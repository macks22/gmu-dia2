...
function filterDocuments($ids) {
  $res = executeSQLSequence($ids, array("select id from document where year(publicationDate)>1970 and title!='' and id in (#)"));
  //print_r($res);
  $retIDs = array();
  foreach($res as $r)
	$retIDs[] = $r["id"];
...
function hasCondition($params) {
  return (isset($params->{"authorID"}) || isset($params->{"personID"}) || isset($params->{"keywordID"}) || isset($params->{"publicationID"}) || isset($params->{"organizationName"}) || isset($params->{"organizationID"}) || isset($params->{"publicationGenreID"}) || isset($params->{"yearRange"}) || isset($params->{"dirID"}) || isset($params->{"POID"}));
}
...
function getAllDocumentIDs() {
  ...
  $maxID = $res[0]["mx"];
  for($i = 1; $i < $maxID+1; $i++) {
	$retArray[] = $i;      //will this leave the maxID out? should we use $i< $maxID+1 or <=$maxID? Xin added +1
  }
  return $retArray;
}
...
function getDocumentIDsByStatusGroupID($id) {
  ...
  $res = executeSQLSequence(array(), array("select statusID as id from group_status where groupID=$id",
...
function getDocumentIDsByFullText($q, $adv = false) {
  $groups = array(array("phet", "physics and chemistry education technology"),
 ...
 // $solr = new Apache_Solr_Service('128.150.141.103', '8983', '/solr');
  ...
  $limit = 9999999;
  if(!$special) {
	// if there is 'and' 'or', leave it as it is, otherwise wrap the whole string with quotes
	if($adv) {
...
function getDocumentIDsByOrganization($name) {
  $retArray = array();
  $SQLs = array("select id from organization where name like '".$name."%' or name like '% ".$name."%'",
...
function getDocumentIDsByOrganizationID($id) {
  $retArray = array();
  $SQLs = array("select id from affiliation where organizationID=$id",
				"select id from Person where affiliationID in (#)");
  $personIDs = executeSQLSequence(array(), $SQLs);
  $pIDs = array();
  ...
  $res = executeSQLSequence($pIDs, array("select distinct documentID as id from document_person where personID in (#)"));
  foreach($res as $item)
	$retArray[] = $item["id"];
  return $retArray;
}
...
function getDocumentIDsByStateID($id) {
  $retArray = array();
  $SQLs = array("select id from address where stateID = $id","select id from affiliation where addressID in (#)",
				"select id from Person where affiliationID in (#)");
  $personIDs = executeSQLSequence(array(), $SQLs);
  $pIDs = array();
  foreach($personIDs as $pID) {
...
function getDocumentIDsByKeyword($name) {
  $retArray = array();
  $SQLs = array("select id from keyword where keyword='".$name."' or keyword like '".$name." %' or keyword like '% ".$name."' or keyword like '% ".$name." %'",
...
function getDocumentIDsByKeywordArray($arr) {
  $name = implode(" ", $arr);
  $retArray = array();
  $SQLs = array("select id from keyword where keyword='".$name."'",
				"select documentID as id from document_keyword where keywordID in (#)");
  $docIDs = executeSQLSequence(array(), $SQLs);
  foreach($docIDs as $item)
	$retArray[] = $item["id"];
...
function getDocumentIDsByPublicationGenreID($id) {
  $retArray = array();
  $docIDs = executeSQLSequence(array(), array("select id from publication where publicationGenreID=$id", "select id from document where publicationID in (#)"));
  foreach($docIDs as $item)
	$retArray[] = $item["id"];
  return $retArray;
}
...
function getDocumentIDsByYearRange($bgnYear, $endYear) {
  ...
  $SQLs = array("select id from document where year(publicationDate)>=$bgnYear AND year(publicationDate)<=$endYear AND publicationID!=$nsfPubID");
  $docIDs = executeSQLSequence(array(), $SQLs);
  foreach($docIDs as $item)
	$retArray[] = $item["id"];
  // For NSF documents
  $SQLs = array("select documentID as id from awardactiveperiod where year(startDate)<=$endYear AND year(expirationDate)>=$bgnYear");
  $docIDs = executeSQLSequence(array(), $SQLs);
  foreach($docIDs as $item)
	$retArray[] = $item["id"];
  return $retArray;
...
function getDocumentIDsByAuthor($name) {
  $retArray = array();
  $SQLs = array("select id from person where firstName='".$name."' OR lastName='".$name."'", "select documentID as id from document_person where personID in (#) AND relationshipID=25");
  $docIDs = executeSQLSequence(array(), $SQLs);
...
function getDocumentIDsByAuthorArray($arr) {
  ...
  $retArray = array();
  $SQLs = array("select id from person where $str", "select documentID as id from document_person where personID in (#) AND relationshipID=2");
  $docIDs = executeSQLSequence(array(), $SQLs);
  for($n = 0; $n < count($docIDs); $n++) 
...
function getDocumentIDsByProgramOfficerArray($arr) {
  ...
  $retArray = array();
  $SQLs = array("select id from person where $str", "select documentID as id from document_person where personID in (#) AND relationshipID=3");
  $docIDs = executeSQLSequence(array(), $SQLs);
  for($n = 0; $n < count($docIDs); $n++) 
	array_push($retArray, $docIDs[$n]["id"]);
  return $retArray;
...
function getDocumentIDsByPI($name) {
  ...
  //$SQLs = array("select award_ID as id from proposalalias where relationship_ID=1", "select awardID as id from AwardID_PIInfo where (first_name='".$name."' OR last_name='".$name."') and awardID not in (#)");
  $docIDs = executeSQLSequence(array(), $SQLs);
  for($n = 0; $n < count($docIDs); $n++) 
	array_push($retArray, $docIDs[$n]["id"]);
  return $retArray;
...
function getDocumentIDsByPIID($id) {
  ...
  foreach($res as $r)
	$dIDs[] = $r["id"];
  $SQLs = array("select id from documentGenre where name='Grant Proposal'", "select id from document where id in (".implode(",", $dIDs).") AND documentGenreID in (#)");
  $docIDs = executeSQLSequence(array(), $SQLs);
  for($n = 0; $n < count($docIDs); $n++) 
	array_push($retArray, $docIDs[$n]["id"]);
  return $retArray;
...
function getDocumentIDsByPOID($id) {
  ...
  $res = executeSQLSequence($ids, array("select documentID as id from document_person where relationshipID=$poID AND personID in (#)"));
  foreach($res as $r)
	$retArray[] = $r["id"];
  return $retArray;
}
...
function getDocumentIDsByPersonID($id) {
  ...
  $retArray = array();
  //print_r($ids);
  $dIDs = executeSQLSequence($ids, array("select documentID as id from document_person where personID in (#)"));
...
function getDocumentIDsByDirID($dirID) {
  $retArray = array();
  $SQLs = array("select id from dir_div_pgm where nsfdirectorateID='$dirID'", "select documentID as id from document_ddp where ddpID in (#)");
  $docIDs = executeSQLSequence(array(), $SQLs);
  for($n = 0; $n < count($docIDs); $n++) 
	array_push($retArray, $docIDs[$n]["id"]);
  return $retArray;
...
function getDocumentIDsByDivID($divID) {
  $retArray = array();
  $SQLs = array("select id from dir_div_pgm where nsfdivisionID='$divID'", "select documentID as id from document_ddp where ddpID in (#)");
  $docIDs = executeSQLSequence(array(), $SQLs);
  for($n = 0; $n < count($docIDs); $n++) 
	array_push($retArray, $docIDs[$n]["id"]);
  return $retArray;
...
function getDocumentIDsByNSFDirectorate($name) {
  $retArray = array();
  $SQLs = array("select id from nsfdirectorate where name='".$name."' OR abbreviation='".$name."'", "select id from dir_div_pgm where nsfdirectorateID in (#)", "select documentID as id from document_ddp where ddpID in (#)");
  $docIDs = executeSQLSequence(array(), $SQLs);
  foreach($docIDs as $d) {
	$retArray[] = $d["id"];
  }
...
function getDocumentIDsByNSFDivision($name) {
  $retArray = array();
  $SQLs = array("select id from nsfdivision where name='".$name."' OR abbreviation='".$name."'","select id from dir_div_pgm where nsfdivisionID in (#)", "select documentID as id from document_ddp where ddpID in (#)");
  $docIDs = executeSQLSequence(array(), $SQLs);
  for($n = 0; $n < count($docIDs); $n++) 
	array_push($retArray, $docIDs[$n]["id"]);
  return $retArray;
...
function getDocumentIDsByNSFProgram($name) {
  $retArray = array();
  $SQLs = array("select id from NSFProgram where name like '$name%' OR name like '$name %' OR name like '% $name %' OR name like '% $name'", "select id from dir_div_pgm where nsfprogramID in (#)", "select documentID as id from document_ddp where ddpID in (#)");
  $docIDs = executeSQLSequence(array(), $SQLs);
  //print_r($docIDs);
  for($n = 0; $n < count($docIDs); $n++) 
	array_push($retArray, $docIDs[$n]["id"]);
...
function getDocumentIDsByNSFProgramID($id) {
  $retArray = array();
  $SQLs = array("select id from dir_div_pgm where nsfProgramID='$id'", "select documentID as id from document_ddp where ddpID in (#)");
  $docIDs = executeSQLSequence(array(), $SQLs);
  for($n = 0; $n < count($docIDs); $n++) 
	array_push($retArray, $docIDs[$n]["id"]);
  return $retArray;
...
function getDocumentIDsByDirDivPgm($dir, $div, $pgm) {
  $retArray = array();
  $SQLs = array("select id from dir_div_pgm where nsfProgramID='$pgm' and nsfdivisionID='$div' and nsfdirectorateID='$dir'", "select documentID as id from document_ddp where ddpID in (#)");
  $docIDs = executeSQLSequence(array(), $SQLs);
//echo $dir.",".$div.",".$pgm."\n";
//exit();
  for($n = 0; $n < count($docIDs); $n++) 
...
function getDocumentIDsByEmail($email) {
  $retArray = array();
  // PI's email
  $SQLs = array("select id from Person where email='".$email."'", "select federal_award_id as id from Proposal where PI_ID in (#)");
  $docIDs = executeSQLSequence(array(), $SQLs);
  for($n = 0; $n < count($docIDs); $n++) 
	array_push($retArray, $docIDs[$n]["id"]);
  // Program officer's email
  $SQLs = array("select id from Person where email='".$email."'",
...
function getDocumentIDsByPhone($phone) {
  $retArray = array();
  // PI's phone
  $SQLs = array("select id from Person where phone='".$phone."'", "select documentID from document_person where personID in (#)");
  $docIDs = executeSQLSequence(array(), $SQLs);
  for($n = 0; $n < count($docIDs); $n++) 
	array_push($retArray, $docIDs[$n]["id"]);
