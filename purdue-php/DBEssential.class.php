...
//exit();
  ...
  public function save($fileName) {
	...
	*/
	...
	$fields = array("full", "docTitle", "docAward", "oriDocIDs", "nonColModDocIDs", "nonModDocIDs", "nonModOriDocIDs", "dupDocIDs","eliminatedPersonIDs", "docAmount", "authorIDs", "dupAuthorIDs", "authorMap", "docAuthors", "authorDocs", "ddpDocs", "docDDP", "poIDs", "docPOs", "poDocs", "personName", "yearDocIDs", "orgPersons", "orgName", "stateOrg", "orgDocs");
	foreach($fields as $f) {
	  if(($f == "docTitle" || $f == "personName" || $f == "docAuthors" || $f == "orgName" || $f == "stateOrg" || $f=="orgDocs" || $f == "poIDs") && $this->full)
		continue;
  ...
  public function load($params, $attrs=null) {
	...
	$caseAttrs = array("full","docTitle","docAward","oriDocIDs","nonColModDocIDs","nonModDocIDs", "nonModOriDocIDs","dupDocIDs","eliminatedPersonIDs","docAmount","authorIDs", "dupAuthorIDs", "docAuthors","authorDocs","authorMap","ddpDocs","docDDP","poIDs","docPOs","poDocs","personName","yearDocIDs","orgPersons", "stateOrg", "orgDocs", "orgName");
	if($attrs==null) {
	  $items = $caseAttrs;
	} else
  ...
  public static function getFileName($params){
	...
	//print_r($paramsArr);
	foreach($paramsArr as $k=>$v) {
	  if($k == "userid" || $k == "mode"|| $k == "overviewmode" || $k == "depth"||(gettype($v) != "string" && gettype($v) != "array" && gettype($v) != "integer"))
		continue;
	  if(gettype($v)=="array")
	  {
	    $filename.= $k."=";
	...
	//$filename = $prePath.$path.$filename.".json";
  ...
  function loadExcludedPersonIDs() {
	...
	//Name Not Available
	$res = executeSQLSequence(array(), array('select id from person where lastname="" or lastname in ("None","Available","DATA NOT AVAILABLE")'));
	foreach($res as $r) {
  ...
  function getDocumentAliasMapByCollab($docIDs) {
	...
	$res = executeSQLSequence($docIDs, array("select documentID1 as id, documentID2 as aliasID from document_document where (documentID1 in (#) or documentID2 in (#)) and relationshipID=$dupID"));
	foreach($res as $r) {
	  $map[$r["id"]] = $r["aliasID"];
	}
	return $map;
  ...
  public static function getPersonAliasMap($pids) {
	$res = executeSQLSequence($pids, array("select id, aliasID from person_person where id in (#) or aliasID in (#)"));
	foreach($res as $r) {
	  $map[$r["id"]] = $r["aliasID"];
	}
	return $map;
  ...
  function loadOrgDocs() {
	...
	$res = executeSQLSequence($persons, array("select id, affiliationID from person where id in (#)"));
	$affs = array();
	foreach($res as $r) {
	...
	$res = executeSQLSequence($affs, array('select id, organizationID from affiliation where id in (#)'));
	foreach($res as $r) {
	  $affID = $r["id"];
  ...
  function loadDDP() {
	$res = executeSQLSequence(array(), array("select id, nsfdirectorateID as dirID, nsfdivisionId as divID, nsfprogramID as pgmID from dir_div_pgm"));
	foreach($res as $r) {
	  $id    = $r["id"];
	  $dirID = $r["dirID"];
	  $divID = $r["divID"];
	  ...
	  $this->ddpID_ddp[$id] = array("dir"=>$dirID,
									"div"=>$divID,
									"pgm"=>$pgmID);
  ...
  function loadConceptPC() {
	$res = executeSQLSequence(array(), array("select parentid as pid, childid as cid from conceptidrelation"));
	foreach($res as $r) {
  ...
  function loadPersonIDNameMap() {
	...
	foreach($res as $r) {
	  ...
	  if($lastName == "" || $lastName == "none" || $lastName=="available" || $lastName=="data not available")
  ...
  function loadDocAmount() {
	$res = executeSQLSequence($this->dupDocIDs, array("select documentID as docID, obligatedToDate as amt from awardamount where documentID in (#)"));
	foreach($res as $r) {
  ...
  function loadDocAward() {
	$res = executeSQLSequence($this->dupDocIDs, array("select documentID as id, originalID from document_source_mapper where documentID in (#)"));
	foreach($res as $r) {
